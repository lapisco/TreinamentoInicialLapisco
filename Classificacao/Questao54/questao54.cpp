...
int main ()
{
	sprintf (endereco, "num_padronizado_GLCM_OpenCv.txt");
	...
	if (opcao == 0)
	{
		...
		k_means (teste, treino, teste_lable, treino_lable, classes, num_linhas,  tamanho_teste, tamanho_treino, vet_classe, temp, atributos, lable);
		delete [] temp;
	}
	if (opcao == 1)
	{
		leave_one_out(normalizado,lable,teste,teste_lable,treino,treino_lable,vet_classe_treino,classes,num_linhas,atributos);
		k_means (teste, treino, teste_lable, treino_lable, classes, num_linhas,  tamanho_teste, tamanho_treino, vet_classe, vet_classe_treino, atributos, lable);
	}
