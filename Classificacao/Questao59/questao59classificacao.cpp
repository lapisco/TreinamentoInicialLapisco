	...
	for (int y = 0; y<tamanho_teste; y++) 
	{
		...
		if (teste_lable_MLP.at<float>(y, maxIndex)!=1.0f)
		{
			...
			for(int class_index=0;class_index<classes;class_index++)
			{
				...
				{
					matrizConfusao.at<float>(maxIndex,class_index)++;//matriz para fazer matriz confusao
                    break;
				}
			}
		}
		...
		{
			cont_acerto++;
			matrizConfusao.at<float>(maxIndex,maxIndex)++;//matriz para fazer matriz confusao
		}
	}
