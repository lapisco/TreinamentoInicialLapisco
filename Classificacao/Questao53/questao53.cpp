...
int main ()
{
	...
	int N;
	printf ("Informe a quantidade de vizinhos: ");
	scanf ("%d", &N);
	...
	if (opcao == 0)
	{
		...
		if (N==1)
			Knn_1 (normalizado,lable,teste,teste_lable,treino,treino_lable,atributos,tamanho_teste,tamanho_treino,classes,num_linhas,opcao,vet_classe,vet_classe_treino);
		else
			Knn_N (normalizado,lable,teste,teste_lable,treino,treino_lable,atributos,tamanho_teste,tamanho_treino,classes,num_linhas,opcao,N ,vet_classe,vet_classe_treino);
	}
	if (opcao == 1)
	{ 
