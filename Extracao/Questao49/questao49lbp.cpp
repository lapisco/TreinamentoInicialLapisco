...
void lbp::ELBP_(const Mat& src, Mat& dst, int radius, int neighbors) {
	...
	for(int n=0; n<neighbors; n++) {
		// sample points
		float x = static_cast<float>(radius) * cos(2.0*3.141592*n/static_cast<float>(neighbors));
		float y = static_cast<float>(radius) * -sin(2.0*3.141592*n/static_cast<float>(neighbors));
		// relative indices
